...
if (!empty($_GET) && array_key_exists('id', $_GET) && !empty($_GET['id'])) {
    ...
    $service = $proprietairesRepository->findAllAndIdUser(intval($_GET['id']));
    if(count($service)>0){
        ...
        $imgs = $proprietairesRepository->findAllImgObj($id_service);
        if(!empty($imgs)) {
        ...
        $infos = $proprietairesRepository->findAllInfObj($id_service);
        if(!empty($infos)) {
        ...
        $lieux = $proprietairesRepository->findAllIdAndLieux($service['id_proprietaire']);
        if(!empty($lieux)) {
