...
if(!$sessionUser->isConnected()) {
    ...
    if (
        ...
        // **********************
        $id_risque = intval($_POST["risque"]);
        $id_lier = intval($_POST["lier"]);
        $id_categ_lieu = intval($_POST["categorie"]);
        $habitable = (array_key_exists("habitable", $_POST) && !empty($_POST["habitable"]) && $_POST["habitable"] == "on");
        $id_lieu = $objRepository->add($objRepository->findIdLieu($id_main), $id_main, $id_categ_lieu, $habitable);
        if(!empty($id_risque)) {
            $objRepository->addModRisque($objRepository->findIdRisque($id_lieu), $id_lieu, $id_risque);
        }
        if(!empty($id_lier)) {
            $objRepository->addModLier($objRepository->findIdLier($id_lieu), $id_lieu, $id_lier);
        }
        // **********************
