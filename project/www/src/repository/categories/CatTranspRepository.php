...
if (!class_exists('CatTranspRepository')) {
    ...
    class CatTranspRepository extends Repository {
        ...
        public function findAll():array {
            return $this->setSql('SELECT * FROM categorie_transport')->fetchAllAssoc();
        }


        public function findAllOrder(bool $orderName = false):array {
            $order = "id_transport DESC";
            if($orderName) {
                $order = "nom";
            }
            return $this->setSql('SELECT * FROM categorie_transport ORDER BY '.$order)->fetchAllAssoc();
        }

        public function findAllId(int $id):array {
            return $this->setSql('SELECT * FROM categorie_transport WHERE id_transport=:id')->setParamInt(":id", $id)->fetchAssoc();
        }
        ...
        public function findAllAndPage(int $nmPage=0, int $nbArtPage=0, bool $orderName = false):array {
            ...
            $sql = 'SELECT * FROM categorie_transport ORDER BY '.$order.$limit.'';
        ...
        public function findAllAndCount():int {
            $sql = 'SELECT * FROM categorie_transport';
            return $this->setSql($sql)
                        ->rowCount();
        }
