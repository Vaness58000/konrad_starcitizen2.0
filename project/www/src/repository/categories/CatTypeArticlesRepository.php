...
if (!class_exists('CatTypeArticlesRepository')) {
    // inculre la classe qui va creer le fichier "errors.log" en cas d'erreur.
    include_once dirname(__FILE__) . '/../../class/classMain/Repository.php';

    // fonction pour faire la connexion a la base de donnes
    class CatTypeArticlesRepository extends Repository {
        ...
        public function findAll():array {
            return $this->setSql('SELECT * FROM gameplay_type AS cat INNER JOIN utilisateurs ON utilisateurs.id_user = cat.id_user_cat')->fetchAllAssoc();
        }
        ...
        public function findAllOrder(bool $orderName = false):array {
            ...
            return $this->setSql('SELECT * FROM gameplay_type AS cat INNER JOIN utilisateurs ON utilisateurs.id_user = cat.id_user_cat ORDER BY '.$order)->fetchAllAssoc();
        }

        public function findAllId(int $id):array {
            return $this->setSql('SELECT * FROM gameplay_type AS cat INNER JOIN utilisateurs ON utilisateurs.id_user = cat.id_user_cat WHERE id=:id')->setParamInt(":id", $id)->fetchAssoc();
        }
        ...
        public function findAllAndPage(int $nmPage=0, int $nbArtPage=0, bool $orderName = false):array {
            ...
            $sql = 'SELECT * FROM gameplay_type AS cat INNER JOIN utilisateurs ON utilisateurs.id_user = cat.id_user_cat ORDER BY '.$order.$limit.'';
        ...
        public function findAllAndCount():int {
            $sql = 'SELECT * FROM gameplay_type AS cat INNER JOIN utilisateurs ON utilisateurs.id_user = cat.id_user_cat';
            return $this->setSql($sql)
                        ->rowCount();
        }
        ...
        public function addMod(int $id, string $name, int $id_user): self {
            if(!empty($id)) {
                $sql = "UPDATE gameplay_type SET nom=:nom WHERE id=:id";
                $this->setSql($sql)
                            ->setParamInt(":id", $id)
                            ->setParam(":nom", $name);
                $this->executeSql();
            } else {
                $sql = "INSERT INTO gameplay_type(nom, id_user_cat) VALUES (:nom, :id_user_cat)";
                $this->setSql($sql)
        ...
        public function delete(int $id): self {
            $sql = "DELETE FROM gameplay_type WHERE id=:id";
