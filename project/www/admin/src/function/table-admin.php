...
if(!function_exists('addImg')) {
    function addImg(int $id, ?string $nom_folder, ?string $src, ?string $alt):?string {
        $add_img = '<div id="divAddImg-'.$id.'" class="addimg multiple-img">'.
                '<img class="obj img-slide-presentation" name="file-'.$id.'" id="img-'.$id.'" src="./../upload/'.$nom_folder.'/'.$src.'" alt="'.$alt.'" />'.
                '<img src="./src/images/trash3-fill.svg" alt="delete image" class="delete-image delete-img delete-multiple-img" id="delete-img-'.$id.'" />'.
            '</div>';
        return $add_img;
    }
}
if(!function_exists('addImgAndPrinc')) {
    function addImgAndPrinc(int $id, ?string $nom_folder, ?string $src, ?string $alt, int $id_img_main = 0):?string {
        ...
        $add_img = '<div id="divAddImg-'.$id.'" class="addimg multiple-img">'.
                '<input type="radio" id="imgMain-'.$id.'" name="img-princ" class="img-princ" value="imgFileMain-'.$id.'"'.$checked.' />'.
                '<label class="img-main" for="imgMain-'.$id.'" ><img src="./src/images/award-fill.svg" alt="main image" /></label>'.
                '<img class="obj img-slide-presentation" name="imgFileMain-'.$id.'" id="imgFileMain-'.$id.'" src="./../upload/'.$nom_folder.'/'.$src.'" alt="'.$alt.'" />'.
                '<img src="./src/images/trash3-fill.svg" alt="delete image" class="delete-image delete-img delete-multiple-img" id="delete-img-'.$id.'" />'.
            '</div>';
        return $add_img;
    }
